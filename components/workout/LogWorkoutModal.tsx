...
export function LogWorkoutModal({ open, onClose, userId, onSuccess }: LogWorkoutModalProps) {
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input placeholder="e.g. Upper body push" {...register("title")} />
            {errors.title && <p className="text-xs text-destructive">{errors.title.message}</p>}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label>Duration (min)</Label>
              <Input type="number" placeholder="45" {...register("duration_minutes")} />
              {errors.duration_minutes && <p className="text-xs text-destructive">{errors.duration_minutes.message}</p>}
            </div>
            <div className="space-y-2">
              <Label>Calories burned</Label>
              <Input type="number" placeholder="300" {...register("calories_burned")} />
            </div>
          </div>
          <div className="space-y-2">
            <Label>Date</Label>
          ...
          <div className="space-y-2">
            <Label>Notes</Label>
            <Textarea placeholder="How did it feel?" {...register("notes")} rows={2} />
          </div>
